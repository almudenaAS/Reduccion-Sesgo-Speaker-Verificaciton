        ...
        for _, row in df_male_train.iterrows():
            if row.n_sample >=muestras:
                for index in range(muestras):
                    ...
                    linea = row.id_user + ' ' + id_audio + '\n'
                    f.write(linea)
            else:
                for index in range(row.n_sample):
                    n=0
                    while True:
                        num = '0' * n + str(index)
                        if len(num)==5: break
    ...
    with open(path+'list_test.txt', 'w', newline='') as f:
        for _, row in df_male_test.iterrows():
            if row.n_sample >= muestras:
                ...
                for index in range(int(row.n_sample-1)):
                    n=0
                    while True:
                        num = '0' * n + str(index)
                        if len(num)==5:
        ...
        for _, row in df_female_test.iterrows():
            if row.n_sample >= muestras:
                ...
                for index in range(row.n_sample-1):
                    n=0
                    while True:
                        num = '0' * n + str(index)
                        if len(num)==5:
