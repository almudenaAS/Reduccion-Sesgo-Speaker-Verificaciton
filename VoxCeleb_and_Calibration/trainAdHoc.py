...
from calibration_funtions import *
...
parser.add_argument('--trn_metafile')
parser.add_argument('--dev_table')
parser.add_argument('--embeddingsTrain')
parser.add_argument('--embeddingsTest')
parser.add_argument('--calTestname')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.eval == True:
        ...
        sc, lab, embeddings = trainer.evaluateFromList(**vars(args))
...
# ('test_map',   help='Map from test ids (first column) to the ids used in the embeddings file (second column).')
            eval_calibration(cuda=False, durs=None, min_dur=0.1, keylist=None, 
                             ptar=0.01, 
                             set='Mozilla-Common-Voice', 
                             raw=False, 
                             level=None, 
                             cluster_priors=None, 
                             fmt='ascii', 
                             model=args.save_path+'/calTrain', 
                             embeddings=args.save_path+'/'args.embeddingsTest, 
                             enroll_map, 
                             test_map, 
                             out_dir=args.save_path+'/'+args.calTestname)
    ...
    for it in range(it,args.max_epoch+1):
        ...
        loss, traineer, embeddings = trainer.train_network(train_loader, verbose=(args.gpu == 0))

        np.savez(args.save_path+'/'+args.embeddingsTrain, embeddings)
    ...
    train_calibration(debug=True, cuda=False, seed=args.seed, configs=['config.json'], 
                          mods=None, 
                          init_subset=None, 
                          restart=True, print_min_loss=True, 
                          trn_embeddings=args.save_path+'/'+args.embeddingsTrain, 
                          trn_metafile=args.trn_metafile, 
                          dev_table=args.dev_table, 
                          out_dir=args.save_path+'/calTrain')
