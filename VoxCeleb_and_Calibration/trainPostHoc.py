...
parser.add_argument('--modelcaltest')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.eval == True:
        ...
        if args.gpu == 0:
            ...
            if args.save_path+'/calTrain.npz' in os.listdir(args.save_path):
                model_name = args.save_path+'/calTrain.npz'
            else:
                model_name=""
            
            model_outscores = args.save_path+'/'+args.modelcaltest
            name_scores = args.save_path+'/scores_Test.npz'
            np.savez(name_scores, sc)

            eval_score_calibration(key=None, ptar=0.01, fmt='npz', scores=name_scores, model=model_name, outscores=model_outscores)
    ...
    for it in range(it,args.max_epoch+1):
        ...
        if it % args.test_interval == 0:
            ...
            sc, lab, _ = trainer.evaluateFromList(**vars(args))
            name_scores = args.save_path+'/scores_Train.npz'
            model_name = args.save_path+'/calTrain.npz'
            np.savez(name_scores, sc)
            
            train_score_calibration(key=None, ptar=0.01, fmt='npz', scores=name_scores, model=model_name)
